...
int main(int argc, char* argv[])
{
...

    {
        ...
        for (int step = 1; step <= stepmax; ++step) 
        {
            ...
            for(MFIter mfi(fnew, TilingIfNotGPU()); mfi.isValid(); ++mfi)   
            {
                const Box& bx = mfi.growntilebox(0);  
                const Array4<Real>& f_new = fnew.array(mfi);
                ...
                amrex::ParallelFor(bx, [=]AMREX_GPU_DEVICE(int i, int j, int k)
                {
                    if(i == 0)
                    {
                        ...
                        for(int q = 0; q < Q; q++)
                        {
                            f_new(x, y, z, q) = feqQian(rhot, {uxt, uyt}, q) + f_new(x1, y1, z1, q) - feqQian(rhot1, {uxt1, uyt1}, q);
                        }                
                    }
            ...
            for(MFIter mfi(fnew,TilingIfNotGPU()); mfi.isValid(); ++mfi)
            {
                ...
                ParallelFor(bx, [=]AMREX_GPU_DEVICE(int i, int j, int k)
                {
                    ...
                    for(int q = 0; q < Q; q++)
                    {
                        f_new(i, j, k, q) = (1 - omega) * f_new(i, j, k, q) + omega * feqQian(rhot, {uxt, uyt}, q);
                    }                
                });
            }
            ...
            if(plot_int > 0 && step % plot_int == 0) 
            {
                ...
                WriteSingleLevelPlotfile(pltfile, velocity, {"u", "v"}, geom, time, step);
            }
        }
