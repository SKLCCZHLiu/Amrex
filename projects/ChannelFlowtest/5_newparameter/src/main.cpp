...
int main(int argc, char* argv[]) {
...

    {
        ...
        // 组装 ba/dm 并调用自由函数打印网格布局
        {
            int nlev = lid.finestLevel() + 1;
            amrex::Vector<amrex::BoxArray> ba(nlev);
            amrex::Vector<amrex::DistributionMapping> dm(nlev);
            for (int lev = 0; lev < nlev; ++lev) {
                ba[lev] = lid.boxArray(lev);
                dm[lev] = lid.DistributionMap(lev);
            }
            lid.PrintGridLayout(ba, dm);
        }
        lid.PrintMeshInfo();
        lid.PrintLbmParm();
