...
void AmrCoreLBM::ReadParameters() {
    ...
    // One-line debug summary to verify parameters from inputs are applied
    if (ParallelDescriptor::IOProcessor()) {
        amrex::Print() << "[DEBUG] amr.blocking_factor: ";
        for (int i = 0; i <= max_level; ++i)
            amrex::Print() << blocking_factor[i] << " ";
        amrex::Print() << " | amr.max_grid_size: ";
        for (int i = 0; i <= max_level; ++i)
            amrex::Print() << max_grid_size[i] << " ";
        amrex::Print() << " | amr.n_error_buf: ";
        for (int i = 0; i <= max_level; ++i)
            amrex::Print() << n_error_buf[i] << " ";
        amrex::Print() << "\n";
    }
}
...
void AmrCoreLBM::Collide(int lev, int n) {
    ...
    for (MFIter mfi(f_old_lev, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        // Box bx = mfi.tilebox();
        // Box interior = Geom(lev).Domain();
        // // 去掉 y 方向两侧各一层（j=0 与 j=hi[1]），保持 x/z 全域
        // interior.growLo(1, -1);
        // interior.growHi(1, -1);
        // bx &= interior;
...
void AmrCoreLBM::WriteCheckpoint(const std::string& chkname, int step, amrex::Real time) const {
    const std::string level_prefix = "Level_";

    if (ParallelDescriptor::IOProcessor()) {
        if (!amrex::UtilCreateDirectory(chkname, 0755)) {
            amrex::CreateDirectoryFailed(chkname);
        }
    }
    ParallelDescriptor::Barrier();

    if (ParallelDescriptor::IOProcessor()) {
        std::ofstream header(chkname + "/Header", std::ios::out | std::ios::trunc);
        header.precision(17);
    ...
    for (int lev = 0; lev <= finest_level; ++lev) {
        ...
        const auto* vel_mf = &velocity[lev];
        const auto* den_mf = &density[lev];

        VisMF::Write(*f_old_mf, MultiFabFileFullPrefix(lev, chkname, level_prefix, "f_old"));
        VisMF::Write(*f_new_mf, MultiFabFileFullPrefix(lev, chkname, level_prefix, "f_new"));
        VisMF::Write(*vel_mf, MultiFabFileFullPrefix(lev, chkname, level_prefix, "velocity"));
        VisMF::Write(*den_mf, MultiFabFileFullPrefix(lev, chkname, level_prefix, "density"));
    }
}

// void AmrCoreLBM::ReadCheckpoint(const std::string& chkname, int& step, amrex::Real& time)
// {
...
//     is >> step;
//     amrex::GotoNextLine(is);
//     is >> time;
//     amrex::GotoNextLine(is);
//     int finest_in_file;
//     is >> finest_in_file;
//     amrex::GotoNextLine(is);
//     AMREX_ALWAYS_ASSERT_WITH_MESSAGE(finest_in_file == finest_level, "Checkpoint finest level mismatch");

//     for (int lev = 0; lev <= finest_level; ++lev) {
//         amrex::BoxArray ba_file;
//         ba_file.readFrom(is);
//         amrex::GotoNextLine(is);
//         AMREX_ALWAYS_ASSERT_WITH_MESSAGE(ba_file == boxArray(lev), "Checkpoint grid layout mismatch");
//     }
...
//         VisMF::Read(velocity[lev], MultiFabFileFullPrefix(lev, chkname, level_prefix, "velocity"));
//         VisMF::Read(density[lev], MultiFabFileFullPrefix(lev, chkname, level_prefix, "density"));
//     }
// }